...
namespace Fable3LUADecompiler
{
    class LuaOpCode
    {
        ...
        public static string OP_Return(LuaFunction function)
        {
            ...
            if (function.instructionPtr == function.sizecode - 1 || (function.instructionPtr >= 1) &&
        ...
        public static string OP_GetUpVal(LuaFunction function)
        {
            ...
            function.writeLine(String.Format("-- r({0}) = upval({1}) // {2}",
                function.currentInstruction.A,
                function.currentInstruction.B,
                function.upvalue[function.currentInstruction.B]));
#endif
            function.Registers[function.currentInstruction.A].value = function.upvalue[function.currentInstruction.B];
            return "";
        }
        ...
        public static string OP_SetUpVal(LuaFunction function)
        {
            ...
            function.writeLine(String.Format("-- upval({0}) = r({1}) // {2}",
                ...
                function.Registers[function.currentInstruction.A].value));
#endif
            function.upvalue[function.currentInstruction.B] = function.Registers[function.currentInstruction.A].value;
            return "";
        }
        ...
        public static string OP_Closure(LuaFunction function)
        {
            ...
            while(i < function.sizecode && function.Instructions[i].OpCode == OpCodes.HKS_OPCODE_DATA)
            {
                if (function.Instructions[i].A == 1)
                {
                    function.SubFunctions[function.currentInstruction.Bx].upvalue.Add(function.Registers[function.Instructions[i].C].value);
                }
                else if (function.Instructions[i].A == 2)
                {
                    function.SubFunctions[function.currentInstruction.Bx].upvalue.Add(function.upvalue[function.Instructions[i].C]);
                }
                else
                {
